...
# convert using the HGNC full table
# refer: https://www.genenames.org/download/ Complete set new TSV: storage.googleapis.com/public-download-files/hgnc/tsv/tsv/hgnc_complete_set.txt
# hgnc_map_file : map table; id_list : list of ids to convert; id_type : type of id_list elements; 
# col_filter : only include HGNC_ESSENTIAL_COLS in resulting df; approved_only : only return rows where HGNC status == 'Approved'
# returns : Pandas.DataFrame with len(id_list) and incuding cols from left_join HGNC on id_type (opt filtered by col_filter)
def conv_hgnc(hgnc_map_file, id_list, id_type, col_filter = True, approved_only = True):
    # check thd id_type
    ...
    # HGNC file parsing
    try:
        df_hgnc = pd.read_csv(hgnc_map_file, sep = '\t')
    except:
        raise ParseMapFileError(f"Failed to parse HGNC map file ({hgnc_map_file}) as Pandas.DataFrame.")
    if df_hgnc.columns.size == 1:
        raise DelimeterError(f"Parsing of HGNC map file ({hgnc_map_file}) resulted in only 1 column - check map file is tab-delimeted.")
    
    # HGNC status filter
    if approved_only:
        df_hgnc[df_hgnc['status'] == 'Approved']
    ...
    # Apply col filter
    if bool(col_filter):
        df_merge = df_merge[HGNC_ESSENTIAL_COLS]
